...
export const Imagen = () => {
  ...
  const {results, output, go}           = useConvert();
  ...
  useEffect(() => {
    if(results.binChunks) {
      let {xValues, yValues} = useGraphVoltages(data, results);
      setGraphCoords({
        x: xValues,
        y: yValues
      });
  ...
  return (
    <div className='d-flex flex-column h-100'>
      ...
      <div className="align-items-center flex-grow-1 d-flex flex-column">
        {
          /* graphCoords.x.length>0 */true ? <div className="d-flex flex-row justify-content-between">
              <img src="" alt="" />
              <Chart
        ...
        <textarea 
          className={"w-100 flex-grow-1 console"+(graphCoords.x.length>0? '' : 'invisiblex borderx-0')}
          name="console" 
          id="console"
          value={output}
          ref={consoleRef}
